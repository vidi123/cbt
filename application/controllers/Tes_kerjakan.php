...
class Tes_kerjakan extends Tes_Controller {
	...
	public function hapus_tes($tesuser_id = null){
	    if(!empty($tesuser_id)){
	       $query = $this->db->where("tessoal_tesuser_id" , $tesuser_id)->from("cbt_tes_soal")->get();
	       
	       
	       foreach($query->result_array() as $tes_soal){
	          $this->db->delete('cbt_tes_soal_jawaban', array('soaljawaban_tessoal_id' => $tes_soal["tessoal_id"])); 
	          
	          $this->db->delete("cbt_tes_soal" , ["tessoal_id" => $tes_soal["tessoal_id"]]);
	       }
	       
	       $this->db->delete("cbt_tes_user" , ["tesuser_id" => $tesuser_id]);
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if($this->cbt_tes_user_model->count_by_status_waktuuser($tes_user_id, $waktuuser)->row()->hasil>0){
                ...
                if($query_soal->num_rows()>0){
                    ...
                    $query_tes = $this->cbt_tes_soal_model->get_by_tessoal_limit($tes_soal_id, 1)->result_array();
                    $soal = $this->cbt_soal_model->get_by_kolom_limit("soal_id", $query_tes[0]["tessoal_soal_id"], 1)->result_array();
                    ...
                    if($query_soal->soal_tipe==1){
                        ...
                        if($query_jawaban->jawaban_benar==1){
                            if(!empty($soal[0]["soal_poin"])){
                                $data_tes_soal['tessoal_nilai'] = $soal[0]["soal_poin"];
                            }else{
