...
class Modul_soal extends Member_Controller {
    ...
    function tambah(){
        ...
        $this->form_validation->set_rules('soal_poin', 'Poin Soal','required|numeric|greater_than_equal_to[1]');
        ...
        if($this->form_validation->run() == TRUE){
        	...
        	if($id_topik=='kosong'){
	        	...
	        	$data["soal_poin"] = $this->input->post("soal_poin" , true);
    ...
    function get_by_id($id=null){
		...
		if(!empty($id)){
			...
			if($query->num_rows()>0){
				...
				$data["soal_poin"] = $query->soal_poin;
			}
		}
    ...
    function get_datatable(){
	    ...
	    foreach ($query as $temp) {			
            ...
            if(!empty($temp->soal_poin)){
                 $record[] = '<div style="text-align: center;">'.$temp->soal_poin.'</div>';
            }else{
                $record[] = '<div style="text-align: center;">0</div>';
            }
