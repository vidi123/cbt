...
class Modul_topik extends Member_Controller {
	...
	function hapusmodul(){
		...
		if(isset($_POST["modul"])){
			...
			if($this->cbt_modul_model->deletemodul($modul_id)){
				echo "<script>
				alert('Berhasil menghapus modul!')
				window.location.href = 'https://cbtsmkn46jakarta.com/index.php/manager/modul_topik';
				</script>";
				$status['pesan'] = 'Modul berhasil dihapus';
			} else{
				$status['pesan'] = 'Gagal Menghapus Modul';
			}
		} else{
        }
	...
	function tambahmodul(){
        ...
        $this->form_validation->set_rules('tambah-modul-modul', 'Nama Modul', 'required|strip_tags');
		if($this->form_validation->run() == TRUE){
        	$data['modul_nama'] = $this->input->post('tambah-modul-modul', true);
            $data['modul_aktif'] = 1;

			if($this->cbt_modul_model->count_by_nama_modul($data['modul_nama'])->row()->hasil>0){
                $status['status'] = 0;
                $status['pesan'] = 'Nama Modul sudah terpakai !';
            }else{
				$this->cbt_modul_model->save($data);
                
                $status['status'] = 1;
                $status['pesan'] = 'Modul berhasil disimpan ';
            }
        }else{
            $status['status'] = 0;
