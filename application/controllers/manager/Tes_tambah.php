        ...
        if(!empty($tes_id)){
            ...
            if($query_tes->num_rows()>0){
                ...
                foreach ($query_soal as $soal) {
                    $all_soal = $this->db->where("soal_topik_id" , $soal->tset_topik_id)->from("cbt_soal")->get()->result();
                    
                    foreach($all_soal as $satu_soal){
                       if(!empty($satu_soal->soal_poin)){
                         $max_score += ($satu_soal->soal_poin);
                       }else{
                        $max_score = $soal->tset_jumlah*$query_tes->tes_score_right;
                       } 
                    }
                }
            }
